...
class Arcane:
    ...
    def _find_summoner(self, traverse_to_summoner: list[tuple[float, float]]) -> bool:
        ...
        match_platform = self._template_finder.search_and_wait(
            templates_platform,
            threshold=0.55,
            time_out=0.5,
            use_grayscale=True,
        ...
        match_summoner = self._template_finder.search_and_wait(
            tempaltes_summoner,
            threshold=0.79,
            time_out=0.5,
            use_grayscale=True,
        ...
        if match_summoner.valid:
            if self._pather.traverse_nodes(
                [461], self._char, time_out=2.2, force_tp=True
            ):
                return True
        else:
    ...
    def battle(self, do_pre_buff: bool) -> Union[bool, tuple[Location, bool]]:
        ...
        for i, data in enumerate(path_arr):
            ...
            self._pather.traverse_nodes(
                [data.calib_node_start], self._char, force_tp=True
            )
            if not self._pather.traverse_nodes_fixed(
                data.static_path_forward, self._char
            ):
                return False
