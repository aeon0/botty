...
class Arcane:
    ...
    def _find_summoner(self, traverse_to_summoner: list[tuple[float, float]]) -> bool:
        ...
        match_platform = template_finder.search_and_wait(templates_platform, threshold=0.55, timeout=0.5, use_grayscale=True, take_ss=False)
        match_summoner = template_finder.search_and_wait(tempaltes_summoner, threshold=0.79, timeout=0.5, use_grayscale=True, take_ss=False)
        if not match_platform.valid and not match_summoner.valid:
            ...
            match_summoner = template_finder.search_and_wait(tempaltes_summoner, threshold=0.79, timeout=1.0, use_grayscale=True, take_ss=False)
