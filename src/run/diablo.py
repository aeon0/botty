...
class Diablo:
    ...
    def _sealdance(self, seal_opentemplates: list[str], seal_closedtemplates: list[str], seal_layout: str) -> bool:
        ...
        while i < 4:
            ...
            wait(1) # do we need to wait (this long / at all)? effectively it adds 5 seconds to the total run duration ;)
        ...
        if self._config.general["info_screenshots"] and not found:
            ...
            #should we add a step here to close the game & start the next run?
        return found
    ...
    def _seal_A1(self) -> bool:
        ...
        #self._char.kill_cs_trash()
        if not self._sealdance(["DIA_A1L2_14_OPEN"], ["DIA_A1L2_14_CLOSED", "DIA_A1L2_14_MOUSEOVER"], seal_layout + "-Seal2"): return False
        if not self._pather.traverse_nodes([613, 615], self._char): return False
        if not self._sealdance(["DIA_A1L2_5_OPEN"], ["DIA_A1L2_5_CLOSED","DIA_A1L2_5_MOUSEOVER"], seal_layout + "-Seal1"): return False
    ...
    def _seal_C1(self) -> bool:
        ...
        self._char.kill_cs_trash()
        self._picked_up_items |= self._pickit.pick_up_items(self._char)
        if not self._pather.traverse_nodes([651, 653], self._char): return False
        self._char.kill_cs_trash()
        self._picked_up_items |= self._pickit.pick_up_items(self._char)
        if not self._pather.traverse_nodes([651], self._char): return False
        if not self._sealdance(["DIA_C1F2_8_OPEN"], ["DIA_C1F2_8_CLOSED", "DIA_C1F_8_MOUSEOVER"], seal_layout + "1"): return False
        if not self._pather.traverse_nodes([652], self._char): return False
        Logger.info("Kill Infector")
        self._char.kill_infector()
        self._picked_up_items |= self._pickit.pick_up_items(self._char)
        if not self._pather.traverse_nodes([653, 654], self._char): return False
