...
class Diablo:
    ...
    def _sealdance(self, seal_opentemplates: list[str], seal_closedtemplates: list[str], seal_layout: str, seal_node: str) -> bool:
        ...
        while i < 4:
            ...
            wait(i*0.5)
            found = TemplateFinder().search_and_wait(seal_opentemplates, threshold=0.75, time_out=0.1, search_type=SearchType.BEST_MATCH, take_ss=False).valid
            if found:
    ...
    def _loop_pentagram(self, path) -> bool:
        ...
        while not found and time.time() - start_time < 15:
            found = TemplateFinder().search_and_wait(templates, threshold=0.83, time_out=0.1, search_type=SearchType.BEST_MATCH, take_ss=False).valid
    ...
    def _entrance_hall(self) -> bool:
        ...
        if TemplateFinder().search_and_wait(templates, threshold=0.8, time_out=0.1, take_ss=False).valid:
            ...
            if not TemplateFinder().search_and_wait(templates, threshold=0.8, time_out=0.5, search_type=SearchType.BEST_MATCH, take_ss=False).valid:
    ...
    def _river_of_flames(self) -> bool:
        ...
        while not found and time.time() - start_time < 10:
            found = TemplateFinder().search_and_wait(templates, threshold=0.8, time_out=0.1, search_type=SearchType.BEST_MATCH, take_ss=False).valid
            if not found:
    ...
    def _layoutcheck(self, sealname:str, boss:str, static_layoutcheck:str, trash_location:str , calibration_node:str, calibration_threshold:str, confirmation_node:str, templates_primary:list[str], templates_confirmation:list[str]):
        ...
        if not TemplateFinder().search_and_wait(templates_primary, threshold =threshold_primary, time_out=0.1, search_type=SearchType.BEST_MATCH, take_ss=False).valid:
            ...
            if not TemplateFinder().search_and_wait(templates_confirmation, threshold=threshold_confirmation, time_out=0.1, search_type=SearchType.BEST_MATCH, take_ss=False).valid:
            ...
            if not TemplateFinder().search_and_wait(templates_confirmation, threshold=threshold_confirmation2, time_out=0.1, search_type=SearchType.BEST_MATCH, take_ss=False).valid:
