...
from ui_components import skills, loading, waypoint, belt, inventory, vendor
...
class Cows:
    ...
    def _legcheck(self) -> bool:
        ...
        legfound = TemplateFinder().search_and_wait(["LEG_INVENTORY"], best_match=True, threshold=0.9,  time_out=0.5, use_grayscale=False, take_ss=False).valid
        #wait(2)
        keyboard.send(Config().char["inventory_screen"])
        ...
        #open stash, search for leg
            ...
            #wait(2)
            if TemplateFinder().search_and_wait(["LEG_INVENTORY"], best_match=True, threshold=0.9,  time_out=0.5, use_grayscale=False, take_ss=False).valid: 
                Logger.info('\033[96m' + "Legcheck: Checking Stash for Leg: found, moving it to inventory" + '\033[0m')
                leg = TemplateFinder().search_and_wait("LEG_INVENTORY", Config().ui_roi["left_inventory"], time_out=3, normalize_monitor=True, take_ss=False) 
                if not leg.valid:
            ...
            #open cube, search for leg
                ...
                if TemplateFinder().search_and_wait(["LEG_INVENTORY"], best_match=True, threshold=0.9,  time_out=0.5, use_grayscale=False, take_ss=False).valid: 
                    ...
                    leg = TemplateFinder().search_and_wait("LEG_INVENTORY", roi=Config().ui_roi["cube_area_roi"], time_out=3, normalize_monitor=True, take_ss=False) 
                    if not leg.valid:
                        Logger.info('\033[96m' + "Legcheck: Failed to transfer Leg to Inventory, aborting run)" + '\033[0m')
                        return False
                    keyboard.send('ctrl', do_release=False)
    ...
    def _open_cow_portal(self)-> bool:
        ...
        TownManager(A1).open_trade_menu(Location.A1_AKARA)
        ...
        wait(0.1, 0.15)
        keyboard.send('ctrl', do_press=False)
        TownManager(A1).close_vendor_screen(Location.A1_AKARA) #transmute().close_cube()
        ...
        legmove = TemplateFinder().search_and_wait("LEG_INVENTORY", roi=Config().ui_roi["right_inventory"], time_out=3, normalize_monitor=True, take_ss=False)
    ...
    def battle(self, do_pre_buff: bool) -> Union[bool, tuple[Location, bool]]:
        ...
        filterimage = TemplateFinder().add_markers(filterimage, threshz, info_ss=True, rect_min_size=3, rect_max_size=5, marker=True) # add markers
