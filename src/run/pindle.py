...
class Pindle:
    ...
    def approach(self, start_loc: Location) -> Union[bool, Location]:
        ...
        if not self._pather.traverse_nodes(
            (loc, Location.A5_NIHLATHAK_PORTAL), self._char
        ):
            return False
        wait(0.5, 0.6)
        found_loading_screen_func = lambda: self._ui_manager.wait_for_loading_screen(
            2.0
        )
        if not self._char.select_by_template(
            ["A5_RED_PORTAL", "A5_RED_PORTAL_TEXT"],
    ...
    def battle(self, do_pre_buff: bool) -> Union[bool, tuple[Location, bool]]:
        ...
        if not self._template_finder.search_and_wait(
            ["PINDLE_0", "PINDLE_1"], threshold=0.65, time_out=20
        ).valid:
            return False
        if do_pre_buff:
            self._char.pre_buff()
        ...
        if self._char.can_teleport():
            ...
            if not self._pather.traverse_nodes(
                (Location.A5_PINDLE_START, Location.A5_PINDLE_SAFE_DIST), self._char
            ):
                return False
