...
class Nihlatak:
    ...
    def battle(self, do_pre_buff: bool) -> Union[bool, tuple[Location, bool]]:
        ...
        template_match = self._template_finder.search_and_wait(
            ["NI1_A", "NI1_B", "NI1_C"], threshold=0.65, time_out=20
        )
        ...
        found_loading_screen_func = (
            lambda: self._ui_manager.wait_for_loading_screen(2.0)
            or self._template_finder.search_and_wait(
                ["NI2_SEARCH_0", "NI2_SEARCH_1"], threshold=0.8, time_out=0.5
            ).valid
        )
        # look for stairs
        if not self._char.select_by_template(
            ["NI1_STAIRS", "NI1_STAIRS_2", "NI1_STAIRS_3", "NI1_STAIRS_4"],
            found_loading_screen_func,
            threshold=0.63,
            time_out=4,
        ):
        ...
        if not self._template_finder.search_and_wait(
            ["NI2_SEARCH_0", "NI2_SEARCH_1", "NI2_SEARCH_2"], threshold=0.8, time_out=20
        ).valid:
            return False
        ...
        check_arr = [
            ...
            EyeCheckData(
                ["NI2_B_1_SAFE_DIST", "NI2_B_1_NOATTACK", "NI2_B_2_NOATTACK"],
                "ni2_b_safe_dist",
                "ni2_circle_b",
                [505],
                [506],
            ...
            EyeCheckData(
                ["NI2_C_1_SAFE_DIST", "NI2_C_1_NOATTACK"],
                "ni2_c_safe_dist",
                "ni2_circle_c",
                [510],
                [511],
            ...
            EyeCheckData(
                ["NI2_D_1_SAFE_DIST", "NI2_D_1_NOATTACK"],
                "ni2_d_safe_dist",
                "ni2_circle_d",
                [515],
                [516, 517],
        ...
        for data in check_arr:
            ...
            # Search for eye
            template_match = self._template_finder.search_and_wait(
                data.template_name, threshold=0.7, best_match=True, time_out=3
            )
            ...
            if template_match.valid and template_match.name.endswith("_SAFE_DIST"):
                ...
                self._pather.traverse_nodes(
                    data.save_dist_nodes, self._char, time_out=2, do_pre_move=False
                )
        ...
        if end_nodes is None:
            ...
            self._pather.traverse_nodes_fixed(
                check_arr[0].destination_static_path_key, self._char
            )
            self._pather.traverse_nodes(
                check_arr[0].save_dist_nodes, self._char, time_out=2, do_pre_move=False
            )
