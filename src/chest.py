    ...
    def open_up_chests(self, time_out: float = 8.0, threshold: float = 0.73) -> bool:
        ...
        while time.time() - start < time_out:
            ...
            if not template_match.valid:
                ...
                wait(0.13, 0.16)
                chest_label = self._template_finder.search(
                    "CHEST_LABEL", self._screen.grab(), threshold=0.85
                )
                if chest_label.valid:
                    Logger.debug(
                        f"Opening {template_match.name} ({template_match.score*100:.1f}% confidence)"
                    )
                    ...
                    wait(0.13, 0.16)
                    locked_chest = self._template_finder.search(
                        "LOCKED", self._screen.grab(), threshold=0.85
                    )
...
if __name__ == "__main__":
    ...
    char = Hammerdin(
        config.hammerdin, config.char, screen, template_finder, ui_manager, pather
    )
