...
if __name__ == "__main__":
    ...
    for filename in os.listdir(args.file_path):
        if filename.endswith(".png"):
            ...
            cv2.imwrite(f"./generated/{filename}_mask.png", out)
            ...
            for count, cntr in enumerate(contours):
                ...
                if contains_black and expected_height and expected_width:
                    ...
                    #     if cv2.countNonZero(difference) == 0:
                    #         break
                    # mask_t=cv2.bitwise_not(marker)
                    # mask_color = cv2.cvtColor(mask_t, cv2.COLOR_GRAY2BGR)
                    # out=cv2.bitwise_and(cropped_item, mask_color)
                    out = cropped_item

                    avg = int(np.average(out))
                    if 10 < avg < 35:
                        #cv2.imwrite(f"./generated/{filename}_{count}_{avg}.png", out)
