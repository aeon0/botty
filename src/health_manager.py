...
class HealthManager:
    def __init__(self, screen: Screen, template_finder: TemplateFinder):
        ...
        self._consumible_manager = None # must be set with the belt manager form bot.py
    ...
    def set_consumible_manager(self, consumible_manager: ConsumibleManager):
            ...
            if ingame_template_match.valid:
                ...
                if (health_percentage < self._config.char["take_rejuv_potion_health"] and last_drink > 1) or \
                   (mana_percentage < self._config.char["take_rejuv_potion_mana"] and last_drink > 2):
                    success_drink_rejuv = self._consumible_manager.drink_potion("rejuv", stats=[health_percentage, mana_percentage])
                    self._last_rejuv = time.time()
                ...
                if not success_drink_rejuv:
                    ...
                    if health_percentage < self._config.char["take_health_potion"] and last_drink > 3.5:
                        self._consumible_manager.drink_potion("health", stats=[health_percentage, mana_percentage])
                        self._last_health = time.time()
                    ...
                    if mana_percentage < self._config.char["take_mana_potion"] and last_drink > 4:
                        self._consumible_manager.drink_potion("mana", stats=[health_percentage, mana_percentage])
                        self._last_mana = time.time()
                # check merc
                ...
                if merc_alive:
                    ...
                    if merc_health_percentage < self._config.char["heal_rejuv_merc"] and last_drink > 4.0:
                        self._consumible_manager.drink_potion("rejuv", merc=True, stats=[merc_health_percentage])
                        self._last_merc_healh = time.time()
                    elif merc_health_percentage < self._config.char["heal_merc"] and last_drink > 7.0:
                        self._consumible_manager.drink_potion("health", merc=True, stats=[merc_health_percentage])
                        self._last_merc_healh = time.time()
...
if __name__ == "__main__":
    ...
    manager.set_consumible_manager(consumible_manager)
