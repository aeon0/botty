...
class HealthManager:
    ...
    def get_health(img: np.ndarray) -> float:
        config = Config()
        health_rec = [
            config.ui_pos["health_left"],
            config.ui_pos["health_top"],
            config.ui_pos["health_width"],
            config.ui_pos["health_height"],
        ]
        health_img = cut_roi(img, health_rec)
        # red mask
        mask1, _ = color_filter(
            health_img, [np.array([0, 110, 20]), np.array([2, 255, 255])]
        )
        mask2, _ = color_filter(
            health_img, [np.array([178, 110, 20]), np.array([180, 255, 255])]
        )
        mask = cv2.bitwise_or(mask1, mask2)
        health_percentage = (float(np.sum(mask)) / mask.size) * (1 / 255.0)
        # green (in case of poison)
        mask, _ = color_filter(
            health_img, [np.array([47, 90, 20]), np.array([54, 255, 255])]
        )
    ...
    def get_mana(img: np.ndarray) -> float:
        config = Config()
        mana_rec = [
            config.ui_pos["mana_left"],
            config.ui_pos["mana_top"],
            config.ui_pos["mana_width"],
            config.ui_pos["mana_height"],
        ]
        mana_img = cut_roi(img, mana_rec)
        mask, _ = color_filter(
            mana_img, [np.array([117, 120, 20]), np.array([121, 255, 255])]
        )
    ...
    def get_merc_health(img: np.ndarray) -> float:
        config = Config()
        health_rec = [
            config.ui_pos["merc_health_left"],
            config.ui_pos["merc_health_top"],
            config.ui_pos["merc_health_width"],
            config.ui_pos["merc_health_height"],
        ]
        ...
        merc_health_percentage = (float(np.sum(health_tresh)) / health_tresh.size) * (
            1 / 255.0
        )
    ...
    def _do_chicken(self, img):
        ...
        if self._config.general["info_screenshots"]:
            self._last_chicken_screenshot = (
                "./info_screenshots/info_debug_chicken_"
    ...
    def start_monitor(self):
        ...
        while self._do_monitor:
            ...
            if self._did_chicken or self._pausing:
                continue
            img = self._screen.grab()
            # TODO: Check if in town or not! Otherwise risk endless chicken loop
            ingame_template_match = self._template_finder.search(
                "WINDOW_INGAME_OFFSET_REFERENCE",
                img,
                roi=self._config.ui_roi["window_ingame_ref"],
                threshold=0.9,
            )
            if ingame_template_match.valid:
                ...
                if (
                    health_percentage < self._config.char["take_rejuv_potion_health"]
                    and last_drink > 1
                ) or (
                    mana_percentage < self._config.char["take_rejuv_potion_mana"]
                    and last_drink > 2
                ):
                    success_drink_rejuv = self._belt_manager.drink_potion(
                        "rejuv", stats=[health_percentage, mana_percentage]
                    )
                    self._last_rejuv = time.time()
                ...
                if not success_drink_rejuv:
                    ...
                    if (
                        health_percentage < self._config.char["take_health_potion"]
                        and last_drink > 3.5
                    ):
                        self._belt_manager.drink_potion(
                            "health", stats=[health_percentage, mana_percentage]
                        )
                        self._last_health = time.time()
                    # give the chicken a 6 sec delay to give time for a healing pot and avoid endless loop of chicken
                    elif (
                        health_percentage < self._config.char["chicken"]
                        and (time.time() - start) > 6
                    ):
                        Logger.warning(
                            f"Trying to chicken, player HP {(health_percentage*100):.1f}%!"
                        )
                        self._do_chicken(img)
                    # check mana
                    last_drink = time.time() - self._last_mana
                    if (
                        mana_percentage < self._config.char["take_mana_potion"]
                        and last_drink > 4
                    ):
                        self._belt_manager.drink_potion(
                            "mana", stats=[health_percentage, mana_percentage]
                        )
                        self._last_mana = time.time()
                # check merc
                ...
                if merc_alive:
                    ...
                    if merc_health_percentage < self._config.char["merc_chicken"]:
                        Logger.warning(
                            f"Trying to chicken, merc HP {(merc_health_percentage*100):.1f}%!"
                        )
                        self._do_chicken(img)
                    if (
                        merc_health_percentage < self._config.char["heal_rejuv_merc"]
                        and last_drink > 4.0
                    ):
                        self._belt_manager.drink_potion(
                            "rejuv", merc=True, stats=[merc_health_percentage]
                        )
                        self._last_merc_healh = time.time()
                    elif (
                        merc_health_percentage < self._config.char["heal_merc"]
                        and last_drink > 7.0
                    ):
                        self._belt_manager.drink_potion(
                            "health", merc=True, stats=[merc_health_percentage]
                        )
                        self._last_merc_healh = time.time()
...
if __name__ == "__main__":
    ...
    keyboard.add_hotkey(
        "f12", lambda: Logger.info("Exit Health Manager") or os._exit(1)
    )
    config = Config()
