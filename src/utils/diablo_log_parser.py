...
for string in simple_string:
    table.rows.append ([string, simple_counter [simple_item_counter]])
    simple_item_counter += 1
    
for prefix in prefix_string:
    complex_item_counter = 0
    for string in complex_string:
        table.rows.append ([prefix + ': ' + string, complex_counter [prefix_counter, complex_item_counter]])
        complex_item_counter += 1
    prefix_counter += 1
table.columns.header = ['String', 'amount']
...
Logger.info ("Parsed info.log - results stored in cs_result.txt in botty root folder")
