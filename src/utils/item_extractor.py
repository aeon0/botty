...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_path", type=str, help="Path to screenshots e.g. C:/data"
    )
        ...
        if filename.endswith(".png"):
            ...
            for count, cluster in enumerate(item_clusters):
                ...
                if gen_truth:
                    ...
                    cv2.waitKey(1)
                    print(
                        f"{count} Input item name and press enter (converts to all caps)..."
                    )
                    item_name = input()
                    if item_name != "":
                        ...
                        if not os.path.exists(f"./ground_truth/{out_filename}.png"):
                            cv2.imwrite(
                                f"./ground_truth/{out_filename}.png",
                                img_clean[y : y + h, x : x + w],
                            )
                    ...
                    avg = int(
                        np.average(cv2.cvtColor(cluster.data, cv2.COLOR_BGR2GRAY))
                    )
                    cv2.imwrite(
                        f"./generated/z_{filename}_{key}_{count}_{avg}.png",
                        cluster.data,
                    )
