...
def get_d2r_game_ip():
    ...
    for connection in psutil.net_connections():
        if (
            ...
            and connection.pid == D2R_pid
            and connection.raddr[1] == 443
        ):
...
if __name__ == "__main__":
    ...
    if config.dclone["region_ips"] != "" and config.dclone["dclone_hotip"] != "":
        ...
        print(
            f"Current Game Server: {get_d2r_game_server_region_by_ip(get_d2r_game_ip())}"
        )
        messenger.send(
            msg=f"Dclone IP Found on {get_d2r_game_server_region_by_ip(get_d2r_game_ip())} on IP: {get_d2r_game_ip()}"
        )
    else:
