...
class GraphicDebuggerController:
    ...
    def start(self):
        ...
        if self._config.advanced_options["graphic_debugger_layer_creator"]:
            self.debugger_thread = threading.Thread(
                target=self.run_debugger_processor,
                daemon=False,
                name="Debugger-processor",
            )
            self.debugger_thread.start()
            # we need to run the ui in the mainloop (tkinter kinda sucks)
            self.ui_thread = threading.Thread(
                target=self.run_debgger_ui, daemon=True, name="Debugger-ui"
            )
            self.ui_thread.start()
        else:
            self.debugger_thread = threading.Thread(
                target=self.run_old_debugger, daemon=False, name="Debugger-processor"
            )
            self.debugger_thread.start()
    ...
    def run_debgger_ui(self):
        ...
        sliderbar_h_l = tk.Scale(
            ...
            length=255,
            variable=self.h_l,
            command=self.update_text_box,
        )
        sliderbar_h_l.grid(column=0, row=0)
        sliderbar_s_l = tk.Scale(
            slidebars_frame,
            label="S - Lower",
            from_=0,
            to=255,
        ...
        slidebar_size = tk.Scale(
            ...
            length=130,
            resolution=0.05,
            variable=self.image_resize_ratio,
        )
        ...
        label = tk.Label(
            frame,
            textvariable=tk.StringVar(value="Current layer value: (you can copy this)"),
        )
        ...
        # Load
        load_layer_label = tk.Label(
            frame,
            textvariable=tk.StringVar(
                value="Load layer: (format: h_min,s_min,v_min,h_max,s_max,v_max)"
            ),
        )
        ...
        button = tk.Button(
            frame,
            text="Add current layer to active",
            command=self.add_current_layer_to_active,
        )
        ...
        # active
        self.layers_active_listbox = tk.Listbox(
            all_layers_frame, width=25, height=20, selectmode=tk.EXTENDED
        )
        self.layers_active_listbox.grid(column=0, row=1, sticky=tk.N)
        active_layers_label_frame = tk.Frame(all_layers_frame)
        layers_active_label = tk.Label(
            active_layers_label_frame, textvariable=tk.StringVar(value="Active Layers")
        )
        layers_active_label.grid(column=0, row=0)
        remove_layer = tk.Button(
            active_layers_label_frame,
            text="Remove Selected",
            command=self.remove_selected_active_layer,
        )
        remove_layer.grid(column=1, row=0)
        active_layers_label_frame.grid(column=0, row=0, sticky=tk.N)
        # existing (from config)
        self.layers_existing_listbox = tk.Listbox(
            all_layers_frame, width=25, height=20, selectmode=tk.EXTENDED
        )
        ...
        layers_existing_label = tk.Label(
            all_layers_frame, textvariable=tk.StringVar(value="Existing Layers")
        )
        ...
        add_all_existing_layers_button = tk.Button(
            layers_button_frame, text="<<", command=self.add_all_existing_layers
        )
        add_all_existing_layers_button.grid(column=0, row=0)
        add_selected_existing_layer_button = tk.Button(
            layers_button_frame, text=" < ", command=self.add_selected_existing_layer
        )
    ...
    def update_text_box(self, new_value=None):
        ...
        self.current_layer_value_text.insert(
            1.0,
            f"{self.h_l.get()},{self.s_l.get()},{self.v_l.get()},{self.h_u.get()},{self.s_u.get()},{self.v_u.get()}",
        )
        self.current_layer_value_text.configure(state="disabled")

        self.current_layer = [
            np.array([self.h_l.get(), self.s_l.get(), self.v_l.get()]),
            np.array([self.h_u.get(), self.s_u.get(), self.v_u.get()]),
        ]
    ...
    def add_selected_existing_layer(self):
        current_selection = [
            self.layers_existing_listbox.get(i)
            for i in self.layers_existing_listbox.curselection()
        ]
        for key, value in self.existing_layers.items():
    ...
    def remove_selected_active_layer(self):
        current_selection = [
            [i, self.layers_active_listbox.get(i)]
            for i in self.layers_active_listbox.curselection()
        ]
    ...
    def update_displayed_layers(self):
        if self.display_only_current_layer.get():
            self.displayed_layers = {}
            self.displayed_layers[
                self.current_layer_name_text.get(1.0, tk.END)
            ] = self.current_layer
        else:
            self.displayed_layers = {}
    ...
    def load_layer(self):
        ...
        if match:
            ...
            print(
                "String in the wrong format, it should match this regex: ^\\d{0,3},\\d{0,3},\\d{0,3},\\d{0,3},\\d{0,3},\\d{0,3}$"
            )
        self.update_text_box()
    ...
    def _update_active_layers_text_box(self):
        ...
        for key, value in self.active_layers.items():
            ...
            concatenated_value = ",".join(
                [str(e) for e in value[0]] + [str(e) for e in value[1]]
            )
        ...
        self.active_layers_values_text.insert(
            1.0,
            "\n".join(
                f"{key}={','.join([str(e) for e in value[0]] + [str(e) for e in value[1]])}"
                for key, value in self.active_layers.items()
            ),
        )
        self.active_layers_values_text.configure(state="disabled")
        ...
        while 1:
            ...
            if self.image_resize_ratio.get() != 1:
                combined_img = cv2.resize(
                    combined_img,
                    None,
                    fx=self.image_resize_ratio.get(),
                    fy=self.image_resize_ratio.get(),
                )
