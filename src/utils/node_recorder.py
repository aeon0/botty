...
class NodeRecorder:
    def __init__(self, screen: Screen, config: Config, run_name):
        ...
        os.system(
            f"cd generated && mkdir templates && cd templates && mkdir {run_name} && cd {run_name} && mkdir nodes"
        )
    ...
    def find_templates(self, img):
        ...
        for key in self._template_finder._templates:
            found = self._template_finder.search(
                key, img, use_grayscale=False, threshold=0.77
            )
            if found.valid:
        ...
        if e.event_type == "down":
            ...
            if e.name == "f8" and self._curr_state == 0:
                ...
                if self._upper_left is None:
                    ...
                    template_path = (
                        f"generated/templates/{self._run_name}/{ref_point_name}.png"
                    )
                    cv2.imwrite(template_path, template_img)
                    self._upper_left = None
                    template_img = load_template(template_path, 1.0, False)
                    self._template_finder._templates[ref_point_name] = [
                        template_img,
            ...
            if e.name == "f9":
                ...
                for key in self.ref_points:
                    rel_loc = (
                        loc_screen[0] - self.ref_points[key][0],
                        loc_screen[1] - self.ref_points[key][1],
                    )
            ...
            if e.name == "f10" or e.name == "f9" and self.ref_points is not None:
                    ...
                    if node_screen_pos is not None:
                        for template_key in self.ref_points:
                            if template_key not in self.nodes[node_idx]:
                                rel_loc = (
                                    node_screen_pos[0]
                                    - self.ref_points[template_key][0],
                                    node_screen_pos[1]
                                    - self.ref_points[template_key][1],
                                )
                ...
                print("")
            print(
                "f8: Create Template | f9: New node at cursor | f10: update nodes with visible templates"
            )
