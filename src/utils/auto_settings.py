...
def get_d2r_folder(config: Config) -> str:
    ...
    if not d2_saved_games:
        ...
        d2_saved_games = (
            f"C:\\Users\\{os.getlogin()}\\Saved Games\\Diablo II Resurrected"
        )
    if not os.path.exists(d2_saved_games):
        print(
            f"Your D2R Saved Games folder could not be found here: {d2_saved_games}, input the correct location:"
        )
...
def backup_settings(config: Config):
    ...
    if os.path.exists(d2_saved_games + "\\Settings_backup.json"):
        r = input(
            "Backup file already exist, are you sure you want to overwrite it? [y] to confirm"
        )
        if r == "y":
            shutil.copyfile(
                d2_saved_games + "\\Settings.json",
                d2_saved_games + "\\Settings_backup.json",
            )
...
def adjust_settings(config: Config):
    ...
    if (
        sct.monitors[monitor_idx]["width"] == 1280
        and sct.monitors[monitor_idx]["height"] == 720
    ):
