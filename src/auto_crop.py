...
if __name__ == "__main__":
    ...
    contour_height = [int(round(num, 0)) for num in [x / 1.5 for x in [14,40]]]
    #contour_width = [int(round(num, 0)) for num in [x / 1.5 for x in [60,700]]]
    contour_width = [int(round(num, 0)) for num in [x / 1.5 for x in [60,1280]]]

    mask = cv2.imread(f"maskD2_1280.png")
    ...
    for filename in os.listdir(args.file_path):
        if filename.endswith(".png"):
            ...
            for count, cntr in enumerate(contours):
                ...
                # if contour contains black pixels and is mostly dark...
                avg=int(np.average(cropped_item))
                if np.min(cropped_item) < 14 and (contour_height[0] < h < contour_height[1]) and (contour_width[0] < w < contour_width[1]):
                    #cropped_item_gray_inv = cv2.bitwise_not(cv2.cvtColor(cropped_item, cv2.COLOR_BGR2GRAY))
                    cropped_item_gray = cv2.cvtColor(cropped_item, cv2.COLOR_BGR2GRAY)
                    #cropped_item_thresh = cv2.threshold(cropped_item_gray, 14, 255, cv2.THRESH_BINARY)

                    #mat = np.matrix(cropped_item_gray)
                    #with open(f"./generated/{filename}_{count}_mask.txt",'wb') as f:
                    #    for line in mat:
                    #        np.savetxt(f, line, fmt='%.2f')

                    mask = cropped_item_gray>14
                    start = finish = 0
                    for col in range(mask.shape[0]):
                        if np.average(mask[:,col]) > 20:
                            start += 1
                        else:
                            break
                    for finish in range(mask.shape[0]):
                        if np.average(mask[:,mask.shape[0]-finish]) > 20:
                            finish += 1
                        else:
                            break

                    cv2.imwrite(f"./generated/{filename}_{count}_mask.png", mask*255)

                    cropped_item = cropped_item[:,start:finish]

                    #cropped_item_thresh = cv2.threshold(cropped_item_gray, 15, 255, cv2.THRESH_BINARY_INV)

                    # crop border https://codereview.stackexchange.com/questions/132914/crop-black-border-of-image-using-numpy
                    # mask = cropped_item_gray_inv>15
                    # m,n = cropped_item_gray_inv.shape
                    # mask0,mask1 = mask.any(0),mask.any(1)
                    # col_start,col_end = mask0.argmax(),n-mask0[::-1].argmax()
                    # row_start,row_end = mask1.argmax(),m-mask1[::-1].argmax()
                    # cropped_item = cropped_item[row_start:row_end,col_start:col_end]
                    cv2.imwrite(f"./generated/{filename}_{count}_crop.png", cropped_item)
                    #print(f"{filename[:-4]}_{count} avg: {avg}")
