...
from ui_components.inventory import inventory_has_items, get_slot_pos_and_img, slot_has_item, keep_item
import itertools
...
def gamble(item_finder: ItemFinder):
    ...
    if template_match.valid:
        ...
        while (gamble_on and gold):
            if (inventory_has_items(Screen().grab(), Config().char["num_loot_columns"], ignore_columns) and inventory_has_items(Screen().grab(),2)):
                gamble_on = False
                close_vendor_screen ()
                break
