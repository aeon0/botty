...
class Config:
    ...
    def parse_item_config_string(key: str = None) -> ItemProps:
        ...
        item_string_as_list = re.split(
            r",\s*(?![^()]*\))", Config._select_val("items", key).upper()
        )
        trim_strs = ["AND(", "OR(", "(", ")", " "]
        clean_string = [
    ...
    def load_data():
        ...
        Config.general = {
            ...
            "restore_settings_from_backup_key": Config._select_val(
                "general", "restore_settings_from_backup_key"
            ),
            "settings_backup_key": Config._select_val("general", "settings_backup_key"),
            "graphic_debugger_key": Config._select_val(
                "general", "graphic_debugger_key"
            ),
            "logg_lvl": Config._select_val("general", "logg_lvl"),
            "randomize_runs": bool(
                int(Config._select_val("general", "randomize_runs"))
            ),
            ...
            if not Config._select_val("general", "discord_status_count")
            else int(Config._select_val("general", "discord_status_count")),
            "discord_status_condensed": bool(
                int(Config._select_val("general", "discord_status_condensed"))
            ),
            "info_screenshots": bool(
                int(Config._select_val("general", "info_screenshots"))
            ),
            "loot_screenshots": bool(
                int(Config._select_val("general", "loot_screenshots"))
            ),
            "d2r_path": Config._select_val("general", "d2r_path"),
            "restart_d2r_when_stuck": bool(
                int(Config._select_val("general", "restart_d2r_when_stuck"))
            ),
            "find_window_via_win32_api": bool(
                int(Config._select_val("general", "find_window_via_win32_api"))
            ),
        }
        ...
        Config.routes = {}
        order_str = Config._select_val("routes", "order").replace(
        ...
        Config.char = {
            ...
            "take_health_potion": float(
                Config._select_val("char", "take_health_potion")
            ),
            "take_mana_potion": float(Config._select_val("char", "take_mana_potion")),
            "take_rejuv_potion_health": float(
                Config._select_val("char", "take_rejuv_potion_health")
            ),
            "take_rejuv_potion_mana": float(
                Config._select_val("char", "take_rejuv_potion_mana")
            ),
            ...
            "fill_shared_stash_first": bool(
                int(Config._select_val("char", "fill_shared_stash_first"))
            ),
            "pre_buff_every_run": bool(
                int(Config._select_val("char", "pre_buff_every_run"))
            ),
            ...
            "atk_len_cs_trashmobs": float(
                Config._select_val("char", "atk_len_cs_trashmobs")
            ),
        ...
        Config.advanced_options = {
            "pathing_delay_factor": min(
                max(
                    int(Config._select_val("advanced_options", "pathing_delay_factor")),
                    1,
                ),
                10,
            ),
            "message_headers": Config._select_val(
                "advanced_options", "message_headers"
            ),
            "message_body_template": Config._select_val(
                "advanced_options", "message_body_template"
            ),
            "message_highlight": bool(
                int(Config._select_val("advanced_options", "message_highlight"))
            ),
            "d2r_windows_always_on_top": bool(
                int(Config._select_val("advanced_options", "d2r_windows_always_on_top"))
            ),
            "graphic_debugger_layer_creator": bool(
                int(
                    Config._select_val(
                        "advanced_options", "graphic_debugger_layer_creator"
                    )
                )
            ),
        }
        ...
        for key in Config._pickit_config["items"]:
            try:
                ...
                if Config.items[key].pickit_type and not os.path.exists(
                    f"./assets/items/{key}.png"
                ):
                    print(
        ...
        for key in Config._game_config["colors"]:
            Config.colors[key] = np.split(
                np.array(
                    [int(x) for x in Config._select_val("colors", key).split(",")]
                ),
                2,
            )
        ...
        for key in Config._game_config["ui_roi"]:
            Config.ui_roi[key] = np.array(
                [int(x) for x in Config._select_val("ui_roi", key).split(",")]
            )

        Config.path = {}
        for key in Config._game_config["path"]:
            Config.path[key] = np.reshape(
                np.array([int(x) for x in Config._select_val("path", key).split(",")]),
                (-1, 2),
            )

        Config.shop = {
            "shop_trap_claws": bool(
                int(Config._select_val("claws", "shop_trap_claws"))
            ),
            "shop_melee_claws": bool(
                int(Config._select_val("claws", "shop_melee_claws"))
            ),
            "shop_3_skills_ias_gloves": bool(
                int(Config._select_val("gloves", "shop_3_skills_ias_gloves"))
            ),
            "shop_2_skills_ias_gloves": bool(
                int(Config._select_val("gloves", "shop_2_skills_ias_gloves"))
            ),
            ...
            "shop_hammerdin_scepters": bool(
                int(Config._select_val("scepters", "shop_hammerdin_scepters"))
            ),
            "speed_factor": float(Config._select_val("scepters", "speed_factor")),
            "apply_pather_adjustment": bool(
                int(Config._select_val("scepters", "apply_pather_adjustment"))
            ),
        }
