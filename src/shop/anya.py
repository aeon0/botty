...
def select_by_template(template_type: str) -> bool:
...
def reset_shop():
    while True:
        success = select_by_template("A5_RED_PORTAL")
...
class AnyaShopper:
    ...
    def __init__(self):
        ...
        # Similar for melee claws but not really worth keeping any less that 11 here since you really want both
        # +2 assassin and a useful other stat, feedback needed.
        ...
        # Preload templates to prevent loading them on each loop.
        self.glove_templates = defaultdict(list)
        self.glove_templates['ias'] = load_template(f'{ASSET_FOLDER}ias_gloves.png', 1.0)
        if Config().shop["shop_3_skills_ias_gloves"]:
            # assets for javazon gloves are mixed up, this one need +3 as in the 1080p version.
            self.glove_templates['gg'].append(load_template(f'{ASSET_FOLDER}gg_gloves.png', 1.0))
        if Config().shop["shop_2_skills_ias_gloves"]:
            self.glove_templates['gg'].append(load_template(f'{ASSET_FOLDER}g_gloves.png', 1.0))
    ...
    def shop_loop(self):
        """Main loop for the Anya shop logic."""
        while True:
            ...
            if ias_glove.valid:
                ...
                for gg_glove in self.glove_templates['gg']:
                    wanted_gloves = TemplateFinder(True).search_v2(template_image=gg_glove, image=img, threshold=0.80)
                    for gloves in wanted_gloves:
                        if gloves.valid:
                            mouse.click(button="right")
                            # TODO: Verify purchases by looking for matches in inventory before and after.
            ...
            if self.look_for_trap_claws or self.look_for_melee_claws:
                ...
                claw_pos = []
                for ck in claw_keys:
                    template_matches = TemplateFinder(True).search_v2(
                        template_name=ck,
                        image=img,
                        roi=self.roi_vendor,
                        threshold=0.88
                    )
                    claw_pos.extend([match.center for match in template_matches])
                # check out each claw.
                for pos in claw_pos:
    ...
    def exit(self):
        ...
        print(
            ...
            f"\t\t{self.ias_gloves_seen} \t\t\t{self.gloves_bought} \t\t{self.claws_evaluated} \t\t\t{self.claws_bought}"
        )
        os._exit(0)
