            ...
            if ias_glove.valid:
                ...
                if self.look_for_plus_3_gloves is True:
                    ...
                    if gg_gloves.valid:
                        ...
                        self._messenger.send({"type": "message", "message": "Bought awesome IAS/+3 gloves!"})
                    ...
                    if self.look_for_plus_2_gloves is True:
                        ...
                        if g_gloves.valid:
                            mouse.click(button="right")
                            self._messenger.send({"type": "message", "message": "Bought some decent IAS/+2 gloves"})
            ...
            if self.look_for_trap_claws is True or self.look_for_melee_claws is True:
                ...
                for pos in claw_pos:
                    ...
                    if trap_score > self.trap_claw_min_score and self.look_for_trap_claws is True:
                        # pick it up
                        mouse.click(button="right")
                        self._messenger.send({"type": "message", "message": f"Bought some terrific trap Claws (score: {trap_score})"})
                    ...
                    if melee_score > self.melee_claw_min_score and self.look_for_melee_claws is True:
                        # pick it up
                        mouse.click(button="right")
                        self._messenger.send({"type": "message", "message": f"Bought some mad melee Claws (score: {melee_score})"})
