...
class BeltManager:
    ...
    def should_buy_pots(self):
        return self._pot_needs["health"] > 8 or self._pot_needs["mana"] > 3
    ...
    def _cut_potion_img(self, img: np.ndarray, column: int, row: int) -> np.ndarray:
        roi = [
            self._config.ui_pos["potion1_x"]
            - (self._config.ui_pos["potion_width"] // 2)
            + column * self._config.ui_pos["potion_next"],
            self._config.ui_pos["potion1_y"]
            - (self._config.ui_pos["potion_height"] // 2)
            - int(row * self._config.ui_pos["potion_next"] * 0.92),
    ...
    def drink_potion(
        self, potion_type: str, merc: bool = False, stats: List = []
    ) -> bool:
        ...
        for i in range(4):
            ...
            if self._potion_type(potion_img) == potion_type:
                ...
                if merc:
                    Logger.debug(
                        f"Give {potion_type} potion in slot {i+1} to merc. HP: {(stats[0]*100):.1f}%"
                    )
                    keyboard.send(f"left shift + {self._config.char[key]}")
                else:
                    Logger.debug(
                        f"Drink {potion_type} potion in slot {i+1}. HP: {(stats[0]*100):.1f}%, Mana: {(stats[1]*100):.1f}%"
                    )
    ...
    def picked_up_pot(self, item_name: str):
        ...
        if item_name in self._item_pot_map:
            self._pot_needs[self._item_pot_map[item_name]] = max(
                0, self._pot_needs[self._item_pot_map[item_name]] - 1
            )
        else:
    ...
    def fill_up_belt_from_inventory(self, num_loot_columns: int):
        ...
        for column, row in itertools.product(range(num_loot_columns), range(4)):
            center_pos, slot_img = UiManager.get_slot_pos_and_img(
