...
class DiscordEmbeds(GenericApi):
    def __init__(self):
        ...
        self._webhook = Webhook.from_url(
            self._config.general["custom_message_hook"],
    ...
    def send_death(self, location, image_path):
        ...
        e = discord.Embed(
            title=f"{self._config.general['name']} has died at {location}",
    ...
    def send_chicken(self, location, image_path):
        ...
        e = discord.Embed(
            title=f"{self._config.general['name']} has chickened at {location}",
    ...
    def send_stash(self):
        e = discord.Embed(
            title=f"{self._config.general['name']} has a full stash!",
            color=Color.dark_grey(),
        )
    ...
    def send_gold(self):
        e = discord.Embed(
            title=f"{self._config.general['name']} is rich!", color=Color.dark_grey()
        )
    ...
    def send_message(self, msg: str):
        ...
        e = discord.Embed(
            title=f"Update:", description=f"```{msg}```", color=Color.dark_teal()
        )
    ...
    def _add_file(self, image_path, image_name):
        try:
            ...
            traceback.print_exc()
            return discord.File(
                "./assets/error/image_not_found.png", filename=image_name
            )
