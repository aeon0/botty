...
class Bot:
    ...
    def __init__(self, game_stats: GameStats):
        ...
        if Config().char["type"] in ["sorceress", "light_sorc"]:
            ...
            self._char: IChar = Hammerdin(Config().hammerdin, self._pather, self._pickit)
        elif Config().char["type"] == "poison_necro":
            self._char: IChar = Poison_Necro(Config().poison_necro, self._pather) #pickit added for diablo
