...
class Bot:
    ...
    def on_maintenance(self):
        ...
        if self._tps_left < random.randint(3, 5) or need_repair or self._config.char["always_repair"] or self._config.char["buy_keys"] and self._config.routes["run_arcane"]:
            if need_repair: Logger.info("Repair needed. Gear is about to break")
            else: Logger.info("Repairing and buying TPs at next Vendor")
            if self._config.char["buy_keys"] and self._config.routes["run_arcane"]:
