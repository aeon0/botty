...
class Bot:
    def __init__(self, screen: Screen, game_stats: GameStats, template_finder: TemplateFinder, pick_corpse: bool = False):
        ...
        if self._config.char["type"] in ["sorceress", "light_sorc"]:
            ...
            self._char: IChar = BlizzSorc(self._config.blizz_sorc, self._screen, self._template_finder, self._ui_manager, self._pather) #pickit added for diablo
            ...
            self._char: IChar = Hammerdin(self._config.hammerdin, self._screen, self._template_finder, self._ui_manager, self._pather, self._pickit)
