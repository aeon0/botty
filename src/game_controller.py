    ...
    def run_bot(self, pick_corpse: bool = False):
        ...
        if do_restart:
            ...
            if self._config.general["custom_message_hook"]:
                messenger.send(
                    msg=f"{self._config.general['name']}: got stuck and will now restart D2R"
                )
            if restart_game(self._config.general["d2r_path"]):
                ...
                if self.setup_screen():
                    ...
                    return self.run_bot(True)
            Logger.error(
                f"{self._config.general['name']} could not restart the game. Quitting."
            )
            if self._config.general["custom_message_hook"]:
                messenger.send(
                    msg=f"{self._config.general['name']}: got stuck and will now quit"
                )
            os._exit(1)
