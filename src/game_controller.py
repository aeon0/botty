...
class GameController:
    ...
    def run_bot(self, pick_corpse: bool = False):
        ...
        self.death_manager.set_callback(
            lambda: self.bot.stop() or kill_thread(self.bot_thread)
        )
        self.health_manager.set_callback(
            lambda: self.bot.stop() or kill_thread(self.bot_thread)
        )
        ...
        while 1:
            ...
            max_game_length_reached = (
                self.game_stats.get_current_game_length()
                > self._config.general["max_game_length_s"]
            )
            if (
                max_game_length_reached
                or self.death_manager.died()
                or self.health_manager.did_chicken()
            ):
                # Some debug and logging
                if max_game_length_reached:
                    Logger.info(
                        f"Max game length reached. Attempting to restart {self._config.general['name']}!"
                    )
                    if self._config.general["info_screenshots"]:
                        cv2.imwrite(
                            "./info_screenshots/info_max_game_length_reached_"
                    ...
                    self.game_stats.log_chicken(
                        self.health_manager._last_chicken_screenshot
                    )
                self.bot.stop()
        ...
        if do_restart:
            ...
            if self._config.general["info_screenshots"]:
                cv2.imwrite(
                    "./info_screenshots/info_could_not_recover_"
                    + time.strftime("%Y%m%d_%H%M%S")
                    + ".png",
                    self.screen.grab(),
                )
            if self._config.general["restart_d2r_when_stuck"]:
                Logger.error(
                    "Could not recover from a max game length violation. Restarting the Game."
                )
                ...
                if restart_game(self._config.general["d2r_path"]):
                    ...
                    if self.setup_screen():
                        ...
                        self.game_recovery = GameRecovery(
                            self.screen, self.death_manager, self.template_finder
                        )
                        return self.run_bot(True)
                Logger.error("Could not restart the game. Quitting.")
                messenger.send_message(
                    "Got stuck and could not restart the game. Quitting."
                )
            else:
                Logger.error(
                    "Could not recover from a max game length violation. Quitting botty."
                )
                if self._config.general["custom_message_hook"]:
    ...
    def start(self):
        ...
        if len(diff) > 0:
            Logger.warning(
                "Your D2R settings differ from the requiered ones. Please use Auto Settings to adjust them. The differences are:"
            )
    ...
    def stop(self):
        ...
        if self.death_monitor_thread:
            kill_thread(self.death_monitor_thread)
        if self.health_monitor_thread:
            kill_thread(self.health_monitor_thread)
        if self.bot_thread:
            kill_thread(self.bot_thread)
    ...
    def start_health_manager_thread(self):
        ...
        self.health_monitor_thread = threading.Thread(
            target=self.health_manager.start_monitor
        )
    ...
    def start_death_manager_thread(self):
        ...
        self.death_monitor_thread = threading.Thread(
            target=self.death_manager.start_monitor
        )
