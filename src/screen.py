...
class Screen:
    ...
    def __init__(self, monitor: int = 0, wait: int = 20):
        ...
        monitor_idx = (
            monitor + 1
        )  # sct saves the whole screen (including both monitors if available at index 0, then monitor 1 at 1 and 2 at 2)
        if len(self._sct.monitors) == 1:
            Logger.error("How do you not have a monitor connected?!")
            os._exit(1)
        if monitor_idx >= len(self._sct.monitors):
            Logger.warning(
                "Monitor index not available! Choose a smaller number for 'monitor' in the param.ini. Forcing value to 0 for now."
            )
            monitor_idx = 1
        self._config = Config()
        ...
        if self._config.general["find_window_via_win32_api"]:
            ...
            if position is None:
                Logger.debug(
                    "Can't find any window owned by D2R.exe falling back to matching via assets. Make sure D2R is in focus and you are on the hero selection screen"
                )
        ...
        if position is not None:
            ...
            if self._config.general["info_screenshots"]:
                cv2.imwrite(
                    "./info_screenshots/error_d2r_window_not_found_"
                    + time.strftime("%Y%m%d_%H%M%S")
                    + ".png",
                    self.grab(),
                )
            Logger.error(
                "Could not find hero selection or template for ingame, shutting down"
            )
            Logger.error(
                "Could not determine window offset. Please make sure you have the D2R window "
    ...
    def _set_window_position(self, offset_x: int, offset_y: int):
        ...
        self._monitor_x_range = (
            self._monitor_roi["left"] + 10,
            self._monitor_roi["left"] + self._monitor_roi["width"] - 10,
        )
        self._monitor_y_range = (
            self._monitor_roi["top"] + 10,
            self._monitor_roi["top"] + self._monitor_roi["height"] - 10,
        )
    ...
    def find_window_via_assets(self, wait: int) -> Tuple[int, int]:
        ...
        template_ingame = load_template(
            f"assets/templates/window_ingame_offset_reference.png", 1.0
        )
        debug_max_val = 0
        start = time.time()
        while time.time() - start < wait:
            ...
            if max_val > 0.84:
                if max_val < 0.93:
                    Logger.warning(
                        f"Your template match score to calc corner was lower then usual ({max_val*100:.1f}% confidence). "
                        + "You might run into template matching issues along the way!"
                    )
                return max_pos
        Logger.error(
            f"The max score that could be found was: ({debug_max_val*100:.1f}% confidence)"
        )
        return None
    ...
    def convert_monitor_to_screen(
        self, screen_coord: Tuple[float, float]
    ) -> Tuple[float, float]:
        return (
            screen_coord[0] - self._monitor_roi["left"],
            screen_coord[1] - self._monitor_roi["top"],
        )
    ...
    def convert_abs_to_screen(
        self, abs_coord: Tuple[float, float]
    ) -> Tuple[float, float]:
        # abs has it's center on char which is the center of the screen
        return (
            (self._monitor_roi["width"] // 2) + abs_coord[0],
            (self._monitor_roi["height"] // 2) + abs_coord[1],
        )

    def convert_screen_to_abs(
        self, screen_coord: Tuple[float, float]
    ) -> Tuple[float, float]:
        return (
            screen_coord[0] - (self._monitor_roi["width"] // 2),
            screen_coord[1] - (self._monitor_roi["height"] // 2),
        )

    def convert_abs_to_monitor(
        self, abs_coord: Tuple[float, float]
    ) -> Tuple[float, float]:
