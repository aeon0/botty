...
class GameRecovery:
    ...
    def go_to_hero_selection(self):
        ...
        while (time.time() - start) < 30:
            ...
            is_loading = True
            while is_loading:
                is_loading = self._template_finder.search(
                    "LOADING", self._screen.grab()
                ).valid
                time.sleep(0.5)
            ...
            if found:
                offline_tab = self._template_finder.search(
                    ["OFFLINE_TAB", "OFFLINE_TAB_DARK"],
                    self._screen.grab(),
                    roi=self._config.ui_roi["offline_tab"],
                    threshold=0.8,
                    best_match=True,
                )
                if offline_tab.valid:
                    ...
                    tab = (
                        offline_tab.position[0] - offline_width,
                        offline_tab.position[1],
                    )
