...
def get_template(key):
    return _all_templates()[key].img_bgr

def _prep_template_and_img(template: Template, img: np.ndarray = None, roi: list, color_match: list = None, use_grayscale: bool = False):
    if color_match:
        template = color_filter(template.img_bgr, color_match)[1]
    else:
        template = template.img_bgr

    if use_grayscale:
        template = template.img_gray
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

def _single_match(template: Template, inp_img: np.ndarray = None, roi: list, color_match: list = None, use_grayscale: bool = False):
    inp_img = inp_img if inp_img is not None else grab()
    template_match = TemplateMatch()
    template_prepped, img_prepped = _prep_template_and_img(template, img, roi, color_match, use_grayscale)

    if not (img_prepped.shape[0] > template_prepped.shape[0] and img_prepped.shape[1] > template_prepped.shape[1]):
        Logger.error(f"Image shape and template shape are incompatible: {template.name}")
    else:
        res = cv2.matchTemplate(img_prepped, template_prepped, cv2.TM_CCOEFF_NORMED, mask = template.alpha_mask)
