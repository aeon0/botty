...
class BlizzSorc(Sorceress):
    ...
    def _ice_blast(
        ...
        delay: tuple[float, float] = (0.16, 0.23),
        spray: float = 10,
    ):
    ...
    def kill_nihlatak(self, end_nodes: list[int]) -> bool:
        ...
        for i in range(atk_sequences):
            ...
            if nihlatak_pos_abs is not None:
                ...
                wait(0.3)
                is_nihl = self._template_finder.search(
                    ["NIHL_BAR"],
                    self._screen.grab(),
                    threshold=0.8,
                    roi=self._config.ui_roi["enemy_info"],
                ).valid
                nihl_immune = self._template_finder.search(
                    ["COLD_IMMUNE", "COLD_IMMUNES"],
                    self._screen.grab(),
                    threshold=0.8,
                    roi=self._config.ui_roi["enemy_info"],
                ).valid
...
if __name__ == "__main__":
    ...
    char = BlizzSorc(
        config.blizz_sorc, config.char, screen, t_finder, ui_manager, pather
    )
    char.kill_council()
