...
class Sorceress(IChar):
    ...
    def pick_up_item(
        ...
        if self._skill_hotkeys["telekinesis"] and any(
            x in item_name for x in ["potion", "misc_gold", "tp_scroll"]
        ):
    ...
    def select_by_template(
        ...
        if not self._skill_hotkeys["telekinesis"] or not telekinesis:
            return super().select_by_template(
                template_type, success_func, time_out, threshold
            )
