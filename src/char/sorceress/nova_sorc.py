...
#-------------------------------------------------------------------------------#   
    ...
    def kill_cs_trash(self, location:str) -> bool:
        ...
        if location in [
            ...
            atk_len = self._char_config["atk_len_cs_trashmobs"] * 2
            pos_m = self._screen.convert_abs_to_monitor((0, 0))
            mouse.move(*pos_m, randomize=80, delay_factor=[0.5, 0.7])
            self._cast_static(0.6)
            self._nova(atk_len)
            ...
            Logger.debug("I have no location argument given for kill_cs_trash(" + location + "), should not happen. Throwing some random novas")
            atk_len = self._char_config["atk_len_cs_trashmobs"] * 2
            pos_m = self._screen.convert_abs_to_monitor((0, 0))
            mouse.move(*pos_m, randomize=80, delay_factor=[0.5, 0.7])
            self._cast_static(0.6)
    ...
    """
    ;sorc attack directions for killing CS trash right (+0,) left (-0,) up (,-0) down (,+0)
    dia_left=-128, 0
    dia_lefttop= -128, -72
    dia_top= 0, -72
    dia_righttop= 128, -72
    dia_right= 128, 0
    dia_rightbottom= 128, 72
    dia_bottom=0, 72
    dia_leftbottom= -128, 72
    """
    def kill_vizier(self, seal_layout:str) -> bool:
        if seal_layout == "A1-L":
            ...
            for _ in range(int(atk_len)):
                self._chain_lightning(cast_pos_abs, spray=20)
