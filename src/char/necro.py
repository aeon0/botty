...
class Necro(IChar):
    def __init__(
        ...
        self._pather.adapt_path(
            (Location.A5_PINDLE_START, Location.A5_PINDLE_SAFE_DIST), [100, 101]
        )
        self._pather.adapt_path(
        ...
        self._pather.adapt_path(
            (Location.A3_TRAV_START, Location.A3_TRAV_CENTER_STAIRS), [900]
        )
    ...
    def _count_revives(self):
        ...
        template_match = self._template_finder.search(
            ["REV_BASE"], img, threshold=0.6, roi=roi, normalize_monitor=True
        )
        if template_match.valid:
    ...
    def _count_skeletons(self):
        ...
        template_match = self._template_finder.search(
            ["SKELE_BASE"], img, threshold=0.6, roi=roi, normalize_monitor=True
        )
        if template_match.valid:
    ...
    def _count_gol(self):
        ...
        template_match = self._template_finder.search(
            ["CLAY"], img, threshold=0.6, roi=roi, normalize_monitor=True
        )
        if template_match.valid:
    ...
    def _summon_stat(self):
        ...
        Logger.info(
            "\33[31m"
            + "Summon status | "
            + str(self._skeletons_count)
            + "skele | "
            + str(self._revive_count)
            + " rev | "
            + self._golem_count
            + " |"
            + "\033[0m"
    ...
    def _revive(
        self, cast_pos_abs: Tuple[float, float], spray: int = 10, cast_count: int = 12
    ):
    ...
    def _raise_skeleton(
        self, cast_pos_abs: Tuple[float, float], spray: int = 10, cast_count: int = 16
    ):
    ...
    def _left_attack_single(
        self, cast_pos_abs: Tuple[float, float], spray: int = 10, cast_count: int = 6
    ):
        keyboard.send(self._char_config["stand_still"], do_release=False)
        if self._skill_hotkeys["skill_left"]:
            keyboard.send(self._skill_hotkeys["skill_left"])
    ...
    def _corpse_explosion(
        self, cast_pos_abs: Tuple[float, float], spray: int = 10, cast_count: int = 8
    ):
        keyboard.send(self._char_config["stand_still"], do_release=False)
    ...
    def _cast_circle(
        self,
        cast_dir: Tuple[float, float],
        cast_start_angle: float = 0.0,
        cast_end_angle: float = 90.0,
        cast_div: int = 10,
        cast_v_div: int = 4,
        cast_spell: str = "raise_skeleton",
        delay: float = 1.0,
        offset: float = 1.0,
    ):
        ...
        for i in range(cast_div):
            ...
            for j in range(cast_v_div):
                circle_pos_screen = self._pather._adjust_abs_range_to_screen(
                    (target * 120.0 * float(j + 1.0)) * offset
                )
                circle_pos_monitor = self._screen.convert_abs_to_monitor(
                    circle_pos_screen
                )
                mouse.move(*circle_pos_monitor, delay_factor=[0.3 * delay, 0.6 * delay])
    ...
    def kill_pindle(self) -> bool:
        ...
        if pindle_pack_kill:
            ...
            wait(self._cast_duration, self._cast_duration + 0.1)
            self._cast_circle(
                cast_dir=[-1, 1],
                cast_start_angle=0,
                cast_end_angle=360,
    ...
    def kill_shenk(self) -> bool:
        ...
        self._pather.traverse_nodes(
            ([146, 147, 148]), self, time_out=1.4, force_tp=True
        )
    ...
    def stairs_S(self):
        ...
        img = self._screen.grab()
        template_match = self._template_finder.search(
            ["TRAV_S", "TRAV_S_1"], img, threshold=0.4, roi=roi, normalize_monitor=True
        )
        if template_match.valid:
    ...
    def stairs_F(self):
        ...
        img = self._screen.grab()
        template_match = self._template_finder.search(
            ["TRAV_F"], img, threshold=0.4, roi=roi, normalize_monitor=True
        )
        if template_match.valid:
            pos = template_match.position
            pos = (pos[0], pos[1])
    ...
    def stairs_W(self):
        ...
        img = self._screen.grab()
        template_match = self._template_finder.search(
            ["TRAV_W", "TRAV_W_1"], img, threshold=0.4, roi=roi, normalize_monitor=True
        )
        if template_match.valid:
            pos = template_match.position
            pos = (pos[0], pos[1])
    ...
    def to_durance(self):
        ...
        img = self._screen.grab()
        template_match = self._template_finder.search(
            ["TRAV_18"], img, threshold=0.3, roi=roi, normalize_monitor=True
        )
        if template_match.valid:
            pos = template_match.position
            pos = (pos[0], pos[1])
    ...
    def to_trav(self):
        ...
        while sel is False:
            ...
            img = self._screen.grab()
            template_match = self._template_finder.search(
                ["TO_TRAV_0"], img, threshold=0.95, roi=roi, normalize_monitor=True
            )
            if template_match.valid:
                pos = template_match.position
                pos = (pos[0], pos[1])
    ...
    def kill_council(self) -> bool:
        ...
        result = self._pather.traverse_nodes(
            (901, 902, 903, 904, 905, 906, 226, 228, 300),
            self,
            force_move=True,
            time_out=2.5,
            threshold=0.55,
        ...
        while result is False:
            ...
            if self._pather.find_abs_node_pos(904, self._screen.grab()):
                # try again
                result = self._pather.traverse_nodes(
                    (904, 905, 906, 226, 228, 300), self, force_move=True, time_out=2.5
                )
            elif self._pather.find_abs_node_pos(905, self._screen.grab()):
                # try again
                result = self._pather.traverse_nodes(
                    (905, 906, 226, 228, 300), self, force_move=True, time_out=2.5
                )
            elif self._pather.find_abs_node_pos(906, self._screen.grab()):
                # try again
                result = self._pather.traverse_nodes(
                    (906, 226, 228, 300), self, force_move=True, time_out=2.5
                )
            elif self._pather.find_abs_node_pos(226, self._screen.grab()):
                # try again
                result = self._pather.traverse_nodes(
                    (226, 228, 300), self, force_move=True, time_out=2.5
                )
                ...
                # try again
                result = self._pather.traverse_nodes(
                    (902, 903, 904, 905, 906, 226, 228, 300),
                    self,
                    force_move=True,
                    time_out=2.5,
                )
