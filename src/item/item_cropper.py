...
class ItemCropper:
    def __init__(self):
        ...
        self._expected_height_range = [
            int(round(num, 0)) for num in [x / 1.5 for x in [14, 40]]
        ]
        self._expected_width_range = [
            int(round(num, 0)) for num in [x / 1.5 for x in [60, 1280]]
        ]
    ...
    def clean_img(self, inp_img: np.ndarray) -> np.ndarray:
        img = inp_img[:, :, :]
        if (
            img.shape[0] == self._hud_mask.shape[0]
            and img.shape[1] == self._hud_mask.shape[1]
        ):
    ...
    def crop(self, inp_img: np.ndarray, padding_y: int = 5) -> list[ItemText]:
        ...
        for key in self._item_colors:
            ...
            blured_img = np.clip(
                cv2.GaussianBlur(
                    filtered_img_gray, self._gaus_filter, cv2.BORDER_DEFAULT
                ),
                0,
                255,
            )
            ...
            for count, cntr in enumerate(contours):
                ...
                expected_height = (
                    1
                    if (
                        self._expected_height_range[0]
                        < h
                        < self._expected_height_range[1]
                    )
                    else 0
                )
                ...
                expected_width = (
                    True
                    if (
                        self._expected_width_range[0]
                        < w
                        < self._expected_width_range[1]
                    )
                    else False
                )
                ...
                if (
                    ...
                    for key2 in self._item_colors:
                        _, extracted_img = color_filter(
                            cropped_item, self._config.colors[key2]
                        )
                        extr_avg = np.average(
                            cv2.cvtColor(extracted_img, cv2.COLOR_BGR2GRAY)
                        )
