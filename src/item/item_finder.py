    ...
    def __init__(self, config: Config):
        ...
        for filename in os.listdir(f"assets/{self._folder_name}"):
            ...
            if filename.endswith(".png"):
                ...
                if blacklist_item or (
                    (item_name in config.items and config.items[item_name].pickit_type)
                    or force_search
                ):
                    ...
                    for key in self._template_color_ranges:
                        _, extracted_template = color_filter(
                            data, self._template_color_ranges[key]
                        )
                        filtered_template = cv2.bitwise_or(
                            filtered_template, extracted_template
                        )
    ...
    def search(self, inp_img: np.ndarray) -> list[Item]:
        ...
        for cluster in item_text_clusters:
            ...
            for key in self._templates:
                ...
                if (
                    cropped_input.shape[1] > template.data.shape[1]
                    and cropped_input.shape[0] > template.data.shape[0]
                ):
                    ...
                    hist_result = cv2.compareHist(
                        template.hist, hist, cv2.HISTCMP_CORREL
                    )
                    same_type = hist_result > 0.0 and hist_result is not np.inf
                    if same_type:
                        result = cv2.matchTemplate(
                            cropped_input, template.data, cv2.TM_CCOEFF_NORMED
                        )
                            ...
                            if best_score is None or max_val > best_score:
                                ...
                                if template.blacklist:
                                    ...
                                    grayscale = cv2.cvtColor(
                                        cropped_item, cv2.COLOR_BGR2GRAY
                                    )
                                    _, mask = cv2.threshold(
                                        grayscale, 0, 255, cv2.THRESH_BINARY
                                    )
                                    hist = cv2.calcHist(
                                        [cropped_item],
                                        [0, 1, 2],
                                    ...
                                    same_type = (
                                        hist_result > 0.65 and hist_result is not np.inf
                                    )
                                    if same_type:
                                        item = Item()
                                        item.center = (
                                            int(
                                                max_loc[0]
                                                + x
                                                + int(template.data.shape[1] * 0.5)
                                            ),
                                            int(
                                                max_loc[1]
                                                + y
                                                + int(template.data.shape[0] * 0.5)
                                            ),
                                        )
                                        item.name = key
                                        item.score = max_val
                                        ...
                                        center_abs = (
                                            item.center[0] - (inp_img.shape[1] // 2),
                                            item.center[1] - (inp_img.shape[0] // 2),
                                        )
    ...
    while 1:
        ...
        for item in item_list:
            ...
            cv2.rectangle(
                img,
                item.roi[:2],
                (item.roi[0] + item.roi[2], item.roi[1] + item.roi[3]),
                (0, 0, 255),
                1,
            )
