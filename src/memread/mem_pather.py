...
class MemPather:
    _instance = None
    _range_x = [-Config().ui_pos["center_x"] + 7, Config().ui_pos["center_x"] - 7]
    _range_y = [-Config().ui_pos["center_y"] + 7, Config().ui_pos["center_y"] - Config().ui_pos["skill_bar_height"] - 33]

    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(MemPather, cls).__new__(cls)
                    ...
                    for p in data["poi"]:
                        if p["label"].startswith(poi):
                            ...
                            if entrance_in_wall:
                                ...
                                if data["map"][ap[1] - 4][ap[0]] == 1:
                                    ap = [p["position"][0], p["position"][1] + 2]
                                elif data["map"][ap[1] + 4][ap[0]] == 1:
                                    ap = [p["position"][0], p["position"][1] - 2]
                                elif data["map"][ap[1]][ap[0] - 4] == 1:
                                    ap = [p["position"][0] + 3, p["position"][1]]
                                elif data["map"][ap[1]][ap[0] + 4] == 1:
                                ...
                                ap = p["position"]
                            pos_monitor = MapAssistApi().world_to_abs_screen(ap)
                            if -640 < pos_monitor[0] < 640 and -360 < pos_monitor[1] < 360:
                                pos_monitor = screen.convert_abs_to_monitor(pos_monitor)
                            else:
                                pos_monitor = None
                            break
                else:
                    pos_monitor = MapAssistApi().world_to_abs_screen(poi)
                    if -640 < pos_monitor[0] < 640 and -360 < pos_monitor[1] < 360:
                        pos_monitor = screen.convert_abs_to_monitor(pos_monitor)
                    else:
                        pos_monitor = None
    ...
    def traverse(self, end: Union[str, tuple[int, int]], char: IChar, randomize: int = 0, do_pre_move: bool = True):
        ...
        while 1:
            time.sleep(0.2) # have to wait a bit... otherwise over casting as always...
            data = MapAssistApi().get_data()
            if data is not None and "map" in data and data["map"] is not None:
                ...
                if r is not None:
                    ...
                    move_to = screen.convert_abs_to_monitor(sc)
...
if __name__ == "__main__":
    ...
    # MemPather().traverse([63, 88], bot._char)
    MemPather().go_to_area("Worldstone Keep Level 3", "TheWorldStoneKeepLevel3")
