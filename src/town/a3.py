...
class A3(IAct):
    ...
    def wait_for_tp(self) -> Location | bool:
        toggle_automap(True)
        template_match = template_finder.search_and_wait("A3_TOWN_AUTOMAP", timeout=20)
        toggle_automap(False)
        if template_match.valid:
            self._pather.traverse_nodes_automap((Location.A3_STASH_WP, Location.A3_STASH_WP), self._char, force_move=True)
            toggle_automap(False)
            return Location.A3_STASH_WP
        return False
