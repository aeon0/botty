...
class A3(IAct):
    ...
    def heal(self, curr_loc: Location) -> Union[Location, bool]:
        if not self._pather.traverse_nodes(
            (curr_loc, Location.A3_ORMUS), self._char, force_move=True
        ):
            return False
    ...
    def open_stash(self, curr_loc: Location) -> Union[Location, bool]:
        if not self._pather.traverse_nodes(
            (curr_loc, Location.A3_STASH_WP), self._char, force_move=True
        ):
            return False
        wait(0.3)
        ...
        if not self._char.select_by_template(
            "A3_STASH", stash_is_open_func, telekinesis=True
        ):
            return False
        return Location.A3_STASH_WP
    ...
    def wait_for_tp(self) -> Union[Location, bool]:
        template_match = self._template_finder.search_and_wait(
            "A3_TOWN_10", time_out=20
        )
        if template_match.valid:
            self._pather.traverse_nodes(
                (Location.A3_STASH_WP, Location.A3_STASH_WP),
                self._char,
                force_move=True,
            )
            return Location.A3_STASH_WP
    ...
    def identify(self, curr_loc: Location) -> Union[Location, bool]:
        if not self._pather.traverse_nodes(
            (curr_loc, Location.A3_STASH_WP), self._char
        ):
            return False
