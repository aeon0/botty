...
class TestGameStats:
    ...
    @pytest.mark.parametrize(
        "item_name, item_should_be_added",
        [
    ...
    def test_log_item_keep(self, item_name: str, item_should_be_added: bool):
        ...
        previous_item_count = len(
            self.game_stats._location_stats["test_location"]["items"]
        )
        self.game_stats.log_item_keep(
            item_name, send_message=False, img=np.zeros([5, 5])
        )
